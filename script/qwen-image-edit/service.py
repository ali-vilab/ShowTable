...
from diffusers import QwenImageEditPipeline, QwenImageEditPlusPipeline, FlowMatchEulerDiscreteScheduler
...
class QwenImage:
    ...
    def setup_pipeline(self) -> None:
        ...
        LORA_MODEL_PATH = "models/Qwen-Image-Lightning/Qwen-Image-Edit-2509/Qwen-Image-Edit-2509-Lightning-8steps-V1.0-bf16.safetensors"
        ...
        for gpu_id in range(WORLD_SIZE):
            ...
            if LORA_MODEL_PATH is not None:
                ...
                pipe = QwenImageEditPlusPipeline.from_pretrained(MODEL_PATH, torch_dtype=torch.bfloat16).to(device=device)
    ...
    def _generate_on_gpu(self, gpu_id: int, input_args: dict) -> List[str]:
        ...
        if isinstance(input_args['image'], str):
            input_args['image'] = self.base64_to_pil(input_args['image'])
        elif isinstance(input_args['image'], list):
            for image in input_args['image']:
                if isinstance(image, str):
                    image = self.base64_to_pil(image)
                input_args['image'].append(image)
    ...
    def base64_to_pil(self, image_b64):
